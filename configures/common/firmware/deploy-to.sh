...
# ============================================================================
# 功能说明:
#   本脚本用于将文件从源目录部署到目标目录，支持通过 MANIFEST 清单文件
#   定义部署规则。执行以下操作：
#     1. 解析命令行参数配置源目录和目标目录
#     2. 读取并解析 MANIFEST 清单文件
#     3. 根据清单文件的指令执行文件复制和目录创建
#     4. 支持设置目标相对路径、清理目录等高级操作
#     5. 记录部署日志
#
# 使用场景:
#   - 部署 CoStrict 系统文件到安装目录
#   - 更新系统配置文件
#   - 批量文件部署和分发
#
# 依赖说明:
...
#
# 安全说明:
#   - 脚本会检查目标目录是否为系统危险目录（如 /, /bin, /usr/bin 等）
#   - 只有指定的源目录中的文件才能被部署
#   - 支持部署前清理选项（--clean），使用需谨慎
# ============================================================================
...
show_usage() {
    cat << EOF
部署脚本：根据 MANIFEST 清单文件将文件从源目录部署到目标目录
