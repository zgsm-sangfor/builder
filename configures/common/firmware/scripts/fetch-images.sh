...
for image in $IMAGES; do
    ...
    # 检查 docker 镜像是否已存在，存在则跳过下载和加载
    if docker image inspect "$image" >/dev/null 2>&1; then
        log "INFO" "镜像已存在，跳过下载和加载: $image"
        ((success_count++))
        continue
    fi
    ...
    if [ "$has_error" = true ]; then
        ...
        # 下载成功后，使用 docker load 加载镜像到本地
        log "INFO" "正在加载镜像: $output_path"
        if docker load -i "$output_path"; then
            log "INFO" "加载成功: $image"
            ((success_count++))
        else
            log "ERROR" "加载失败: $image"
            rm -f "$output_path"
            ((error_count++))
        fi
    fi
...
log "INFO" "下载并加载完成 - 成功: ${success_count} 个"
