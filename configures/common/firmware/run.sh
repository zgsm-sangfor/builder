...
# ============================================================================
# 功能说明:
#   本脚本用于启动 CoStrict 系统服务，执行以下操作：
#     1. 检查核心依赖项（目录、配置文件等）
#     2. 生成环境变量文件 (.env)
#     3. 启动 Docker Compose 服务（包括所有容器）
#     4. 输出服务访问地址和管理页面 URL
#
# 使用场景:
#   - 首次启动系统（需先执行 init.sh）
...
#   - 需要 bash 4.0 或更高版本
#   - 需要先执行 init.sh 进行系统初始化
#   - 需要 docker-compose.yml 配置文件
#
# 启动的服务包括:
#   - APISIX (API 网关)
#   - Casdoor (认证服务)
#   - Portal (前端服务)
#   - Costrict Admin Backend (后端管理)
#   - PostgreSQL (数据库)
#   - Redis (缓存)
#   - ETCD (配置存储)
#   - 其他微服务组件
...
show_usage() {
    cat << EOF
启动和管理 CoStrict 系统服务：启动、停止、重启、查看状态等操作
