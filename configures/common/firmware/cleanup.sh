...
# 注意事项:
#   - 卸载操作不可逆，请谨慎操作
#   - 默认删除数据目录，如需保留请使用 --keep-data 或单独保留数据库 --keep-db 
...
选项:
    ...
    --keep-data        保留数据目录，不删除数据
    --keep-db          保留数据目录下的数据库文件，仅保留 costrict-admin.db
    -y, --yes         跳过确认提示，直接执行卸载
    -h, --help        显示帮助信息

示例:
    $0                                          # 卸载系统（保留默认设置）
    $0 --backend /opt/costrict                  # 卸载目录为 /opt/costrict 的系统
...
confirm_uninstall() {
    ...
    log "WARN" "  2. 移除所有系统服务"
    log "WARN" "  3. 删除安装目录: $BACKEND_DIR"
    if [ "$KEEP_DATA" = false ]; then
        if [ "$KEEP_DB" = true ]; then
            log "INFO" "  6. 删除数据目录（保留 costrict-admin.db）: $DATA_DIR"
        else
            log "WARN" "  6. 删除数据目录: $DATA_DIR"
        fi
    else
        log "INFO" "  6. 保留数据目录: $DATA_DIR"
    fi
...
stop_docker_services() {
    log "INFO" "开始停止 Docker 服务..."
    
    # 检查安装目录是否存在
    if [[ ! -d "$BACKEND_DIR" ]]; then
        log "WARN" "安装目录不存在，跳过停止服务: $BACKEND_DIR"
        return 0
    fi
    
    # 切换到安装目录
    cd "$BACKEND_DIR" || {
        log "WARN" "无法切换到安装目录: $BACKEND_DIR"
        return 0
    }
    ...
    log "INFO" "执行 docker-compose down..."
...
remove_backend_dir() {
    ...
    if [[ -d "$BACKEND_DIR" ]]; then
        log "INFO" "删除目录: $BACKEND_DIR"
        sudo rm -rf "$BACKEND_DIR"
        log "INFO" "已删除: $BACKEND_DIR"
    else
        log "INFO" "目录不存在: $BACKEND_DIR"
    fi
}

remove_data_dir() {
    if [ "$KEEP_DATA" = true ]; then
        log "INFO" "根据 --keep-data 选项，保留数据目录: $DATA_DIR"
        return 0
    fi
    
    # 备份数据库文件（如果启用 --keep-db 选项）
    local db_file="${DATA_DIR}/data/costrict-admin/costrict-admin.db"
    local bak_file="${BACKEND_DIR}/costrict-admin.db.bak"
    ...
    if [[ -d "$DATA_DIR" ]]; then
        log "INFO" "删除目录: $DATA_DIR"
        sudo rm -rf "$DATA_DIR"
        log "INFO" "已删除: $DATA_DIR"
...
main() {
    ...
    if [[ -f "$env_file" ]]; then
        log "INFO" "读取安装环境配置: $env_file"
        source "$env_file"
        if [[ -n "${COSTRICT_BACKEND_DIR:-}" ]]; then
            BACKEND_DIR="$COSTRICT_BACKEND_DIR"
            log "INFO" "从配置文件读取安装目录: $BACKEND_DIR"
        fi
        if [[ -n "${COSTRICT_DATA_DIR:-}" ]]; then
            DATA_DIR="$COSTRICT_DATA_DIR"
            log "INFO" "从配置文件读取数据目录: $DATA_DIR"
        fi
    fi
    # 确认卸载
    confirm_uninstall
    ...
    log "INFO" "  - 系统服务已注销"
    log "INFO" "  - 安装目录已删除: $BACKEND_DIR"
    if [ "$KEEP_DATA" = false ]; then
        if [ "$KEEP_DB" = true ]; then
            log "INFO" "  - 数据目录已删除（已保留 costrict-admin.db）: $DATA_DIR"
            log "INFO" "  - 数据库文件已恢复到: ${DATA_DIR}/data/costrict-admin/costrict-admin.db"
        else
            log "INFO" "  - 数据目录已删除: $DATA_DIR"
        fi
    else
        log "INFO" "  - 数据目录已保留: $DATA_DIR"
    fi
}
