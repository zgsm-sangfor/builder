...
#
# 怎么在.env中定义build-components.sh可上传的环境？如下：
# 
...
build_conf() {
    ...
    # 解析platforms数组
    local platform_count=$(echo "$platforms_json" | jq 'length')
    # Get target from components directory JSON file
    local target=$(jq -r ".target // empty" "components/${package_name}.json")
    if [ -z "$target" ] || [ "$target" = "null" ] || [ "$target" = "" ]; then
        echo "Error: 'target' not found for package '${package_name}' in components/${package_name}.json!"
        exit 1
    fi
    ...
    echo "All configuration built successfully for package: $package_name"
}
...
build_package() {
    local package="$1"
    local package_config_file="components/${package}.json"
    ...
    # 从components目录的对应JSON文件中获取指定包的版本号和路径
...
# Function to get package type from components directory JSON file
get_package_type() {
    local package_name=$1
    local package_config_file="components/${package_name}.json"
...
# Function to get package description from components directory JSON file
...
cleanup_old_versions() {
    ...
    # 从components目录的JSON文件中获取指定包的版本号
...
process_package() {
    local package_name=$1
    # 检查包配置文件是否存在
    if [ ! -f "components/${package_name}.json" ]; then
        echo "Error: Package configuration file 'components/${package_name}.json' does not exist!"
        exit 1
    fi
...
process_packages() {
    ...
    # 如果包列表为空，从components目录读取所有JSON文件
    if [ ${#package_list[@]} -eq 0 ]; then
        echo "No packages specified, reading from components directory..."
...
if [ -z "$(ls -A components/*.json 2>/dev/null)" ]; then
    echo "Error: No JSON files found in 'components' directory!"
    exit 1
fi
